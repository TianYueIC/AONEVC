...
void CSourceView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(egde.PtInRect(point))
	{
		...
		if( line <= m_iTotalLine) 
		{
			...
			if(!g_pSSX16BEmulator->IsReady()) return;
			
			if(g_pSSX16BEmulator->BreakPoint(m_iFileIndex,line))
				Invalidate();
		}
	}
...
void CSourceView::UpdateFirstLine()
{
	if(m_iFileIndex < 0) return;
	if(!g_pSSX16BEmulator->IsReady()) return;
	int ip = g_pSSX16BEmulator->GetIP();
	unsigned info = g_pSSX16BEmulator->GetDebugInfo(ip);
...
void CSourceView::DrawBreakPoint(CDC *pDC)
{
	...
	while(1)
	{
		unsigned info = g_pSSX16BEmulator->GetNextDebugInfo(m_iFileIndex,index);
		if(index >= 0 )
		{
...
{	if(g_pSSX16BEmulator->BreakPoint(m_iFileIndex,m_iCursorLine))Invalidate(); }
void CSourceView::OnUpdateMenuBreakPoint(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pSSX16BEmulator->IsReady() && m_iFileIndex >=0); }

void CSourceView::OnMenuGotoCursor() 
{	g_pSSX16BEmulator->GoToCursor(m_iFileIndex,m_iCursorLine);}
void CSourceView::OnUpdateMenuGotoCursor(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pSSX16BEmulator->IsReady() && m_iFileIndex >=0 && g_pSSX16BEmulator->IsStoped()); }
