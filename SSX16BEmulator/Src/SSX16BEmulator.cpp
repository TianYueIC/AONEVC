...
#include "..\Include\SSX16BEmulator.h"
#include "..\..\Library\include\utility.h"
#include "..\..\SSX16B\include\mainfrm.h"
...
CSSX16BEmulator * g_pSSX16BEmulator = NULL;

//CSSX16BEmulator::CSSX16BEmulator(int rom_size,int ram_size,CEmluatorDoc* pEmluatorDoc) : CCoreThread()
CSSX16BEmulator::CSSX16BEmulator(int rom_size,int ram_size,int ioType)
				: CEmulator(16 * 1024)
{
	m_iRAMSize = 256;
	m_iROMSize = 16 * 1024;	
...
CSSX16BEmulator::~CSSX16BEmulator()
{
}
void	CSSX16BEmulator::SetIOType(int ioType)
{
	m_iIOtype = ioType;
}

BOOL CSSX16BEmulator::InitInstance()
{
	return CCoreThread::InitInstance();
}

int CSSX16BEmulator::ExitInstance()
{
...
void CSSX16BEmulator::DisAsm(int ip,CString& str)
{
	DSPDisAsm(ip,m_ROM[ip] | (m_ROM[ip+1] << 16),str);
}

void  CSSX16BEmulator::GetMap(CDSPMap & Map)
{
	////
	Map.m_RAM = m_RAM;
...
void  CSSX16BEmulator::SetMap(CDSPMap &Map)
{
	if(IsRunning()) return;
	////
	m_RAM = Map.m_RAM;
...
void  CSSX16BEmulator::simEnd()
{
	g_Hardwre.SimEnd();
}
BOOL CSSX16BEmulator::Step()
{
	BOOL bBreak = FALSE;
...
BOOL  CSSX16BEmulator::IsM(int code)
{
	int c = code & 0xc000;
...
void  CSSX16BEmulator::M_PreOp(int code)
{
	if(IsM(code))
	{
...
void  CSSX16BEmulator::M_PostOp(int code)
{
	if(IsM(code))
	{
		//CRM &m_RM = * CVHardware::m_pRM;
...
unsigned int CSSX16BEmulator::M_Read(int code)
{
	//################################
...
BOOL CSSX16BEmulator::M_Write(int code,unsigned data)
{
	BOOL bBreak = FALSE;
	//################################
	#define RA  ((code & 0xc0 )>> 6)
...
BOOL CSSX16BEmulator::LoadFile(FILE* fp,FILE* fpdbg)
{
	fseek(fp,0,SEEK_END);
...
void CSSX16BEmulator::SetProjectName(CString &str)
{
	m_strProjectName = str;
	g_Hardwre.SetProjectName(str);
	m_RAM.Setup(str);
}
void CSSX16BEmulator::GetSetupMessage(CString &str)
{
	g_Hardwre.GetIntMessage(str);
	m_RAM.GetSetupMessage(str);
}
void  CSSX16BEmulator::SendTraceOut(int iTracePos,int *pTraceIP, char *pTraceOut)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
	pMainFrame->SendTraceOut(iTracePos,pTraceIP,pTraceOut);
}
void  CSSX16BEmulator::SendErrorMsg(CString & strMsg)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
