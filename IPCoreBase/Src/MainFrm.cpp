						 ...
						 m_ExpRamBar(4)
{
	...
	m_iIOtype = 32;
	g_pEmulator = new CDSPEmluator(0,0,m_iIOtype);
	g_pEmulator->GetMap(*m_pMap);
	g_pEmulator->GetMap(*m_pOldMap);

	m_strPathName = AfxGetApp()->GetProfileString("DSPCORE","PathName","C:\\Work");
	m_strPathName = AfxGetApp()->GetProfileString("DSPCORE","ProjectName","DSP");
	m_iOutListType = AfxGetApp()->GetProfileInt("DSPCORE","OutputListType",10);
}

CMainFrame::~CMainFrame()
{
	g_pEmulator->KillThread();
	g_pEmulator = NULL;
	delete m_pMap;
	delete m_pOldMap;
	delete m_pTraceOut;
...
void CMainFrame::SendErrorMsg(CString & strMsg)
{
	*m_pOldMap = *m_pMap;
	...
	BOOL bOK;
	CString & strFileName = g_pEmulator->GetSourceFileName(m_pMap->m_IP,bOK);
	if(bOK)
	{
		CDSPCoreDoc* pDoc = (CDSPCoreDoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	int file_count  = g_pEmulator->GetSourceFileNum();
	for(int i = 0; i < file_count; i ++)
	{
		CString & strFileName = g_pEmulator->SourceFileName(i);
		CDSPCoreDoc* pDoc = (CDSPCoreDoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	pCmdUI->Enable(g_pEmulator->IsReady());
...
void CMainFrame::CompileAllFile()
{
	...
	ClearMessage();
	CMultiDocTemplate* pDocTemplate = ((CDSPCoreApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
	{
		CDSPCoreDoc* pDoc = (CDSPCoreDoc*)pDocTemplate->GetNextDoc(pos);
...
{	pCmdUI->Enable(g_pEmulator->IsReady() && g_pEmulator->IsRunning()); }

void CMainFrame::OnMenuClearAllbreakpoint() 
{	
	g_pEmulator->ClearBreakPoint(); 
	CMultiDocTemplate* pDocTemplate = ((CDSPCoreApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
...
{	pCmdUI->Enable(g_pEmulator->IsReady() && g_pEmulator->IsStoped()); }
...
{	pCmdUI->Enable(g_pEmulator->IsLoad() && g_pEmulator->IsStoped()); }
...
{
	AfxGetApp()->WriteProfileString("DSPCORE","ProjectName",m_strProjectName);
	AfxGetApp()->WriteProfileString("DSPCORE","PathName",m_strPathName);
	AfxGetApp()->WriteProfileInt("DSPCORE","OutputListType",m_iOutListType);
