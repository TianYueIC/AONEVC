...
void CSourceView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(egde.PtInRect(point))
	{
		...
		if( line <= m_iTotalLine) 
		{
			...
			if(!g_pEmulator->IsReady()) return;
			
			if(g_pEmulator->BreakPoint(m_iFileIndex,line))
				Invalidate();
		}
	}
...
void CSourceView::UpdateFirstLine()
{
	if(m_iFileIndex < 0) return;
	if(!g_pEmulator->IsReady()) return;
	int ip = g_pEmulator->GetIP();
	unsigned info = g_pEmulator->GetDebugInfo(ip);
...
void CSourceView::DrawBreakPoint(CDC *pDC)
{
	...
	while(1)
	{
		unsigned info = g_pEmulator->GetNextDebugInfo(m_iFileIndex,index);
		if(index >= 0 )
		{
...
{	if(g_pEmulator->BreakPoint(m_iFileIndex,m_iCursorLine))Invalidate(); }
void CSourceView::OnUpdateMenuBreakPoint(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pEmulator->IsReady() && m_iFileIndex >=0); }

void CSourceView::OnMenuGotoCursor() 
{	g_pEmulator->GoToCursor(m_iFileIndex,m_iCursorLine);}
void CSourceView::OnUpdateMenuGotoCursor(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pEmulator->IsReady() && m_iFileIndex >=0 && g_pEmulator->IsStoped()); }
