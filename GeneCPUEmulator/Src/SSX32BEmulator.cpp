...
#include "..\..\SSX32B\include\mainfrm.h"
...
CSSX32BEmluator * g_pSSX32BEmulator = NULL;

//CSSX32BEmluator::CSSX32BEmluator(int rom_size,int ram_size,CEmluatorDoc* pEmluatorDoc) : CCoreThread()
CSSX32BEmluator::CSSX32BEmluator(int rom_size,int ram_size,int ioType)
				: CEmulator(16 * 1024)
{
	m_iRAMSize = 256;
	m_iROMSize = 16 * 1024;	
...
CSSX32BEmluator::~CSSX32BEmluator()
{
}
void	CSSX32BEmluator::SetIOType(int ioType)
{
	m_iIOtype = ioType;
}

BOOL CSSX32BEmluator::InitInstance()
{
	return CCoreThread::InitInstance();
}

int CSSX32BEmluator::ExitInstance()
{
...
void CSSX32BEmluator::DisAsm(int ip,CString& str)
{
	DSPDisAsm(ip,m_ROM[ip] | (m_ROM[ip+1] << 16),str);
}

void  CSSX32BEmluator::GetMap(CDSPMap & Map)
{
	////
	Map.m_RAM = m_RAM;
...
void  CSSX32BEmluator::SetMap(CDSPMap &Map)
{
	if(IsRunning()) return;
	////
	m_RAM = Map.m_RAM;
...
void  CSSX32BEmluator::simEnd()
{
	g_Hardwre.SimEnd();
}
BOOL CSSX32BEmluator::Step()
{
	BOOL bBreak = FALSE;
...
BOOL  CSSX32BEmluator::IsM(int code)
{
	int c = code & 0xc000;
...
void  CSSX32BEmluator::M_PreOp(int code)
{
	if(IsM(code))
	{
...
void  CSSX32BEmluator::M_PostOp(int code)
{
	if(IsM(code))
	{
		//CRM &m_RM = * CVHardware::m_pRM;
...
unsigned int CSSX32BEmluator::M_Read(int code)
{
	//################################
...
BOOL CSSX32BEmluator::M_Write(int code,unsigned data)
{
	BOOL bBreak = FALSE;
	//################################
	#define RA  ((code & 0xc0 )>> 6)
...
BOOL CSSX32BEmluator::LoadFile(FILE* fp,FILE* fpdbg)
{
	fseek(fp,0,SEEK_END);
...
void CSSX32BEmluator::SetProjectName(CString &str)
{
	m_strProjectName = str;
	g_Hardwre.SetProjectName(str);
	m_RAM.Setup(str);
}
void CSSX32BEmluator::GetSetupMessage(CString &str)
{
	g_Hardwre.GetIntMessage(str);
	m_RAM.GetSetupMessage(str);
}
void  CSSX32BEmluator::SendTraceOut(int iTracePos,int *pTraceIP, char *pTraceOut)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
	pMainFrame->SendTraceOut(iTracePos,pTraceIP,pTraceOut);
}
void  CSSX32BEmluator::SendErrorMsg(CString & strMsg)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
