...
class CSSX16AEmulator : public CEmulator
{
    CSSX16AEmulator();
    CSSX16AEmulator(const CSSX16AEmulator &ref);
    CSSX16AEmulator &operator=(const CSSX16AEmulator &ref);
private:
	unsigned	m_RD[4];
	unsigned	m_RA[4];
	unsigned	m_SPmax;
...
public:
	...
	CSSX16AEmulator(int rom_size,int ram_size,int ioType);
	virtual ~CSSX16AEmulator();
...
extern CSSX16AEmulator * g_pSSX16AEmulator;
#endif