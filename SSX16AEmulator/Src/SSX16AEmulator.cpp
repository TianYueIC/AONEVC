...
#include "..\Include\SSX16AEmulator.h"
#include "..\..\Library\include\utility.h"
#include "..\..\SSX16A\include\mainfrm.h"
...
CSSX16AEmulator * g_pSSX16AEmulator = NULL;

//CSSX16AEmulator::CSSX16AEmulator(int rom_size,int ram_size,CEmluatorDoc* pEmluatorDoc) : CCoreThread()
CSSX16AEmulator::CSSX16AEmulator(int rom_size,int ram_size,int ioType)
				: CEmulator(16 * 1024)
{
	m_iRAMSize = 256;
	m_iROMSize = 16 * 1024;	
...
CSSX16AEmulator::~CSSX16AEmulator()
{
}
void	CSSX16AEmulator::SetIOType(int ioType)
{
	m_iIOtype = ioType;
}

BOOL CSSX16AEmulator::InitInstance()
{
	return CCoreThread::InitInstance();
}

int CSSX16AEmulator::ExitInstance()
{
...
void CSSX16AEmulator::DisAsm(int ip,CString& str)
{
	DSPDisAsm(ip,m_ROM[ip] | (m_ROM[ip+1] << 16),str);
}

void  CSSX16AEmulator::GetMap(CDSPMap & Map)
{
	////
	Map.m_RAM = m_RAM;
...
void  CSSX16AEmulator::SetMap(CDSPMap &Map)
{
	if(IsRunning()) return;
	////
	m_RAM = Map.m_RAM;
...
void  CSSX16AEmulator::simEnd()
{
	g_Hardwre.SimEnd();
}
BOOL CSSX16AEmulator::Step()
{
	BOOL bBreak = FALSE;
...
BOOL  CSSX16AEmulator::IsM(int code)
{
	int c = code & 0xc000;
...
void  CSSX16AEmulator::M_PreOp(int code)
{
	if(IsM(code))
	{
...
void  CSSX16AEmulator::M_PostOp(int code)
{
	if(IsM(code))
	{
		//CRM &m_RM = * CVHardware::m_pRM;
...
unsigned int CSSX16AEmulator::M_Read(int code)
{
	//################################
...
BOOL CSSX16AEmulator::M_Write(int code,unsigned data)
{
	BOOL bBreak = FALSE;
	//################################
	#define RA  ((code & 0xc0 )>> 6)
...
BOOL CSSX16AEmulator::LoadFile(FILE* fp,FILE* fpdbg)
{
	fseek(fp,0,SEEK_END);
...
void CSSX16AEmulator::SetProjectName(CString &str)
{
	m_strProjectName = str;
	g_Hardwre.SetProjectName(str);
	m_RAM.Setup(str);
}
void CSSX16AEmulator::GetSetupMessage(CString &str)
{
	g_Hardwre.GetIntMessage(str);
	m_RAM.GetSetupMessage(str);
}
void  CSSX16AEmulator::SendTraceOut(int iTracePos,int *pTraceIP, char *pTraceOut)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
	pMainFrame->SendTraceOut(iTracePos,pTraceIP,pTraceOut);
}
void  CSSX16AEmulator::SendErrorMsg(CString & strMsg)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
