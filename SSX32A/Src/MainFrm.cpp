...
CMainFrame::CMainFrame():m_RegisterBar(2),
						 m_MemeryBar(3),
						 m_ExpRamBar(4)
{
	...
	g_pSSX32AEmulator = new CSSX32AEmulator();
	g_pSSX32AEmulator->GetMap(*m_pMap);
	g_pSSX32AEmulator->GetMap(*m_pOldMap);

	m_strPathName = AfxGetApp()->GetProfileString("SSX32A","PathName","C:\\Work");
	m_strPathName = AfxGetApp()->GetProfileString("SSX32A","ProjectName","DSP");
	m_iOutListType = AfxGetApp()->GetProfileInt("SSX32A","OutputListType",0);//10
}

CMainFrame::~CMainFrame()
{
	g_pSSX32AEmulator->KillThread();
...
void CMainFrame::SendErrorMsg(CString & strMsg)
{
	*m_pOldMap = *m_pMap;
	...
	BOOL bOK;
	CString & strFileName = g_pSSX32AEmulator->GetSourceFileName(m_pMap->m_IP,bOK);
	if(bOK)
	{
		CSSX32ADoc* pDoc = (CSSX32ADoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	int file_count  = g_pSSX32AEmulator->GetSourceFileNum();
	for(int i = 0; i < file_count; i ++)
	{
		CString & strFileName = g_pSSX32AEmulator->SourceFileName(i);
		CSSX32ADoc* pDoc = (CSSX32ADoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	pCmdUI->Enable(g_pSSX32AEmulator->IsReady());
...
void CMainFrame::CompileAllFile()
{
	...
	ClearMessage();
	CMultiDocTemplate* pDocTemplate = ((CSSX32AApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
	{
		CSSX32ADoc* pDoc = (CSSX32ADoc*)pDocTemplate->GetNextDoc(pos);
...
{	g_pSSX32AEmulator->Break();}
void CMainFrame::OnUpdateMenuBreak(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pSSX32AEmulator->IsReady() && g_pSSX32AEmulator->IsRunning()); }

void CMainFrame::OnMenuClearAllbreakpoint() 
{	
	g_pSSX32AEmulator->ClearBreakPoint(); 
	CMultiDocTemplate* pDocTemplate = ((CSSX32AApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
...
{	pCmdUI->Enable(g_pSSX32AEmulator->IsReady() && g_pSSX32AEmulator->IsStoped()); }

void CMainFrame::OnMenuGo() 
{	g_pSSX32AEmulator->Go(); }
...
{	pCmdUI->Enable(g_pSSX32AEmulator->IsLoad() && g_pSSX32AEmulator->IsStoped()); }
...
{	g_pSSX32AEmulator->StopEmulator(); }
...
{
	AfxGetApp()->WriteProfileString("SSX32A","ProjectName",m_strProjectName);
	AfxGetApp()->WriteProfileString("SSX32A","PathName",m_strPathName);
	AfxGetApp()->WriteProfileInt("SSX32A","OutputListType",m_iOutListType);
