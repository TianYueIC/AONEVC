...
#include "..\..\SSX32A\include\mainfrm.h"
...
CDSPMap::CDSPMap()
{
	...
	m_iRAMSize = PRIVATE_RAM_SIZE + SYSTEM_RAM_SIZE;
	m_pRAMData = new unsigned [m_iRAMSize];

	int size = PRIVATE_EXPRAM_SIZE + SYSTEM_EXPRAM_SIZE;
	m_ExpRam = new MUint[size];
}
CDSPMap::~CDSPMap()
{
	delete []m_pRAMData;
...
void CDSPMap::operator = (CDSPMap & ram)
{
	...
	m_DB0 = ram.m_DB0;
/*	int i;
	for (i = 0; i < PRIVATE_EXPRAM_SIZE; i++)
		m_ExpRam[i] = ram.m_ExpRam[ram.m_iTaskID*PRIVATE_EXPRAM_SIZE + i];
	for (i = 0; i < SYSTEM_EXPRAM_SIZE; i++)
		m_ExpRam[PRIVATE_EXPRAM_SIZE+i] = 
			ram.m_ExpRam[TASK_NUM*PRIVATE_EXPRAM_SIZE + i];
*/
}
...
CSSX32AEmulator * g_pSSX32AEmulator = NULL;

CSSX32AEmulator::CSSX32AEmulator() : CEmulator(16 * 1024)
{

	g_pSSX32AEmulator = this;
	...
	m_RA[2]  = 0;
	m_RA[3]  = 0;
	m_SPmax = g_SSX32A_ram.m_iRAMSize;
...
CSSX32AEmulator::~CSSX32AEmulator()
{
}
BOOL CSSX32AEmulator::InitInstance()
{
	return CCoreThread::InitInstance();
}

int CSSX32AEmulator::ExitInstance()
{
	g_pSSX32AEmulator = NULL;
	return CEmulator::ExitInstance();
}

void  CSSX32AEmulator::GetMap(CDSPMap & Map)
{
	////
	g_SSX32A_ram.GetMap(Map);
...
void CSSX32AEmulator::reset()
{
	...
	m_bNeedSend = FALSE;
	g_Hardwre.Reset(err, errmsg);
	g_SSX32A_ram.Reset(err, errmsg);
	if(err)
	{
		m_strSendErrorMsg += errmsg;
		m_bNeedSend = TRUE;
	}
...
void  CSSX32AEmulator::simEnd()
{
	g_Hardwre.SimEnd();
}

BOOL CSSX32AEmulator::Step()
{
	BOOL bBreak = FALSE;
	//###############################################################
	#define RA    RD
	...
	unsigned code      = g_SSX32A_rom.ReadROMCode(m_IP,err,errmsg);
	if (err) 
	{
	...
	else if ((code & 0xc100) == 0x0000) //if (Flag%d) goto far 0x%x;
	{
		...
		if (g_Hardwre.ReadRF(RFlag)) 
			m_IP = g_SSX32A_rom.ReadROMCode(m_IP,err,errmsg)+ ((code & 0xff) << 16);
		else  m_IP++;
	}
	else if ((code & 0xc000) == 0x4000)//if (Flag%d) goto near 0x%x;
	{
		...
		bBreak = M_Write(code, m_iLastIP, err, errmsg);
	else if ((code & 0xffc0) == 0x39c0)//Return
	{
		unsigned ip = g_SSX32A_ram.ReadRAM(m_RA[3], err, errmsg);
		m_IP       =  ip & 0x00FFFFFF;
		m_RA[3]   += RETN;
		g_Hardwre.StackOut(ip);
	}
	else if ((code & 0xf9ff) == 0xf8fa)		//	push RA
		bBreak = M_Write(code,m_RA[RA], err, errmsg);
	else if ((code & 0xf9ff) == 0xf9f9)		//  pop RA
		m_RA[RA] = M_Read(code, err, errmsg);
	else if ((code & 0xf9ff) == 0xa8fa)		//  push RD
		bBreak = M_Write(code,m_RD[RD], err, errmsg);
	else if ((code & 0xf9ff) == 0xa9f9)		//	pop RD
		m_RD[RD] = M_Read(code, err, errmsg);
	else if ((code & 0xf900) == 0x3100)		//	RD = N
		m_RD[RD] = g_SSX32A_rom.ReadROMCode(m_IP++, err, errmsg) + (N<<16);
	else if ((code & 0xf900) == 0x2900)		//RD = ROM[N]
		m_RD[RD] = g_SSX32A_rom.ReadROMData(N,err,errmsg);
	else if ((code & 0xf900) == 0x2100)// op1 M
	{
		bUpdateZero = TRUE;
		if(1 < op1) bUpdateFlag = TRUE;
		unsigned int val =  M_Read(code, err, errmsg);
		if (0 == op1) val++;//++
		else if (1 == op1) val--;//--
		else if (2 == op1) //<<
		{
		...
		bBreak = M_Write(code,val, err, errmsg);
		if(val) new_nzero = 1;
		else new_nzero = 0;
	}
	...
	{
		if(RW) m_RD[RD] = g_SSX32A_ram.ReadRAM(N, err, errmsg);
		else g_SSX32A_ram.WriteRAM(N, m_RD[RD], err, errmsg);
	}
	else if ((code & 0xf800) == 0xa800) //RD <<=>> M_Addressing
	{
		if (RW) m_RD[RD] = M_Read(code, err, errmsg) ;
		else bBreak = M_Write(code, m_RD[RD], err, errmsg);
	}
	else if ((code & 0xf800) == 0xf800) //RA <<=>> M_Addressing
	{		
		if (RW) m_RA[RA] = M_Read(code, err, errmsg);
		else bBreak = M_Write(code, m_RA[RA], err, errmsg);
	}
	else if ((code & 0xc000) == 0x8000) //RD OP2 M[...]
	{
		...
		if (RW)
		{
			val =  M_Read(code, err, errmsg);
			val0 = m_RD[RD];
		}
		else
		{
			val0 =  M_Read(code, err, errmsg);
			val  = m_RD[RD];
		}
		...
		if (RW) m_RD[RD] = val0;
		else bBreak = M_Write(code,val0, err, errmsg);
	...
	else if ((code & 0xc000) == 0xc000) //RP <=>  RD RM M
	{
		if (RW) 
		{
			unsigned r = M_Read(code, err, errmsg);
			g_Hardwre.WriteRP(RP, r, err, errmsg);
		}
		else 
		{
			unsigned r = g_Hardwre.ReadRP(RP, err, errmsg);
			bBreak = M_Write(code, r, err, errmsg);
		}
		CString str;
	...
	{
		if(code & 0x0200) g_Hardwre.SetRB(code&0xff, err, errmsg);
		else  g_Hardwre.ClearRB(code&0xff, err, errmsg);
	}
	else 
	{
	...
	//=====对于影响标志位的操作更新标志位=======================
	// g_Hardwre.WriteRF(17,((m_RD[0]>>8)&1)); 2006/10/30 xp
	if (bUpdateFlag == TRUE)
	{
...
BOOL  CSSX32AEmulator::IsM(int code)
{
	int c = code & 0xc000;
...
void  CSSX32AEmulator::M_PreOp(int code)
{
	if(IsM(code))
	{
...
void  CSSX32AEmulator::M_PostOp(int code)
{
	if(IsM(code))
	{
		//CRM &m_RM = * CVHardware::m_pRM;
...
unsigned int CSSX32AEmulator::M_Read(int code, bool &err, CString &errmsg)
{
	//################################
	...
	if(RA == 2 && g_Hardwre.m_bRA2RAM == FALSE) return g_SSX32A_rom.ReadROMData(addr, err, errmsg);
	return g_SSX32A_ram.ReadRAM(addr, err, errmsg);
...
BOOL CSSX32AEmulator::M_Write(int code,unsigned data, bool &err, CString &errmsg)
{
	BOOL bBreak = FALSE;
	//################################
	#define RA  ((code & 0xc0 )>> 6)
	...
	if(RA != 2 || g_Hardwre.m_bRA2RAM) g_SSX32A_ram.WriteRAM(addr,data, err, errmsg);
	return bBreak;
	//################################
	#undef RA
	#undef RD
...
BOOL CSSX32AEmulator::LoadFile(FILE* fp,FILE* fpdbg, CString &errmsg)
{
	fseek(fp,0,SEEK_END);
	int iCodeSize = (int)ftell(fp)/2;

	if(iCodeSize >= g_SSX32A_rom.m_iROMSize)
	{
		errmsg.Format("调试的代码长度为%d，ROM空间只有%d，不能运行",
			iCodeSize,g_SSX32A_rom.m_iROMSize);
		return FALSE;
	}
	fseek(fp,0,SEEK_SET);
	g_SSX32A_rom.LoadFile(fp);
	return LoadDbgFile(fpdbg,iCodeSize,dbghead,sizeof(dbghead),errmsg);
}

void CSSX32AEmulator::SetProjectName(CString &str)
{
	m_strProjectName = str;
	g_Hardwre.SetProjectName(str);
}
void  CSSX32AEmulator::SendTraceOut(int iTracePos,int *pTraceIP, char *pTraceOut)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
	pMainFrame->SendTraceOut(iTracePos,pTraceIP,pTraceOut);
}
void  CSSX32AEmulator::SendErrorMsg(CString & strMsg)
{
	CMainFrame * pMainFrame = (CMainFrame*) AfxGetApp()->GetMainWnd();
