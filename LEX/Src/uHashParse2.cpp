...
void CuHashParse::DeclarationConst(ValPoint pV)
{
...
void CuHashParse::GenCode(int i)
{
...
int  CuHashParse::FindLabel(CString & name)
{
...
int  CuHashParse::FindSub(CString & name)
{
	CLabelItem * pTmp = m_pFirstLabel;
	while(pTmp)
	{
...
int  CuHashParse::FindConst(CString & name)
{
...
void CuHashParse::DisAsm(int code,CString& str)
{
	HashDisAsm(code,str);
}

void	CuHashParse::AddCode(int c, int t)
{
	if(m_iCurCmdPos < 512)
	{
...
// 4=> 行间注释
// 8=> 地址
void	CuHashParse::WriteListFile(FILE * fp,int type)
{
	char c[20];
	CString str;
	for(int i = 0; i < 512; i ++)
...
void CuHashParse::WriteCodeFile(FILE * fp)
{
...
void	CuHashParse::WriteDbgFile(FILE * fp)
{
	//写文件头
