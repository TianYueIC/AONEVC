...
#include "..\include\SSX32BConfigDlg.h"
...
CMainFrame::CMainFrame():m_RegisterBar(VIEWBAR_REGISTER),
						 m_MemeryBar(VIEWBAR_MEMERY),
						 m_ExpRamBar(VIEWBAR_EXPRAM)
{
	...
	m_iIOtype = 32;
	g_pSSX32BEmulator = new CSSX32BEmluator(0,0,m_iIOtype);
	g_pSSX32BEmulator->GetMap(*m_pMap);
	g_pSSX32BEmulator->GetMap(*m_pOldMap);

	m_strPathName = AfxGetApp()->GetProfileString("SSX32B","PathName","C:\\Work");
	m_strPathName = AfxGetApp()->GetProfileString("SSX32B","ProjectName","DSP");
	m_iOutListType = AfxGetApp()->GetProfileInt("SSX32B","OutputListType",10);
}

CMainFrame::~CMainFrame()
{
	g_pSSX32BEmulator->KillThread();
	g_pSSX32BEmulator = NULL;
	delete m_pMap;
	delete m_pOldMap;
	delete m_pTraceOut;
	delete m_pTraceIP;
...
void CMainFrame::SendErrorMsg(CString & strMsg)
{
	*m_pOldMap = *m_pMap;
	...
	BOOL bOK;
	CString & strFileName = g_pSSX32BEmulator->GetSourceFileName(m_pMap->m_IP,bOK);
	if(bOK)
	{
		CSSX32BDoc* pDoc = (CSSX32BDoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	int file_count  = g_pSSX32BEmulator->GetSourceFileNum();
	for(int i = 0; i < file_count; i ++)
	{
		CString & strFileName = g_pSSX32BEmulator->SourceFileName(i);
		CSSX32BDoc* pDoc = (CSSX32BDoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	pCmdUI->Enable(g_pSSX32BEmulator->IsReady());
...
void CMainFrame::CompileAllFile()
{
	...
	ClearMessage();
	CMultiDocTemplate* pDocTemplate = ((CSSX32BApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
	{
		CSSX32BDoc* pDoc = (CSSX32BDoc*)pDocTemplate->GetNextDoc(pos);
...
{	g_pSSX32BEmulator->Break();}
void CMainFrame::OnUpdateMenuBreak(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pSSX32BEmulator->IsReady() && g_pSSX32BEmulator->IsRunning()); }

void CMainFrame::OnMenuClearAllbreakpoint() 
{	
	g_pSSX32BEmulator->ClearBreakPoint(); 
	CMultiDocTemplate* pDocTemplate = ((CSSX32BApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
...
{	pCmdUI->Enable(g_pSSX32BEmulator->IsReady() && g_pSSX32BEmulator->IsStoped()); }

void CMainFrame::OnMenuGo() 
{	g_pSSX32BEmulator->Go(); }
...
{	pCmdUI->Enable(g_pSSX32BEmulator->IsLoad() && g_pSSX32BEmulator->IsStoped()); }
...
{	g_pSSX32BEmulator->StopEmulator(); }
...
{
	AfxGetApp()->WriteProfileString("SSX32B","ProjectName",m_strProjectName);
	AfxGetApp()->WriteProfileString("SSX32B","PathName",m_strPathName);
	AfxGetApp()->WriteProfileInt("SSX32B","OutputListType",m_iOutListType);
...
{	
	m_iIOtype = 32;
	g_pSSX32BEmulator->SetIOType(32);
}
...
{	
	m_iIOtype = 16;
	g_pSSX32BEmulator->SetIOType(16);
}
