						 ...
						 m_ExpRamBar(4)
{
	...
	m_iIOtype = 32;
	g_pIPCoreEmulator = new CIPCoreEmulator(0,0,m_iIOtype);
	g_pIPCoreEmulator->GetMap(*m_pMap);
	g_pIPCoreEmulator->GetMap(*m_pOldMap);

	m_strPathName = AfxGetApp()->GetProfileString("IPCore","PathName","C:\\Work");
	m_strPathName = AfxGetApp()->GetProfileString("IPCore","ProjectName","DSP");
	m_iOutListType = AfxGetApp()->GetProfileInt("IPCore","OutputListType",10);
}

CMainFrame::~CMainFrame()
{
	g_pIPCoreEmulator->KillThread();
...
void CMainFrame::SendErrorMsg(CString & strMsg)
{
	*m_pOldMap = *m_pMap;
	...
	BOOL bOK;
	CString & strFileName = g_pIPCoreEmulator->GetSourceFileName(m_pMap->m_IP,bOK);
	if(bOK)
	{
		CIPCoreDoc* pDoc = (CIPCoreDoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	int file_count  = g_pIPCoreEmulator->GetSourceFileNum();
	for(int i = 0; i < file_count; i ++)
	{
		CString & strFileName = g_pIPCoreEmulator->SourceFileName(i);
		CIPCoreDoc* pDoc = (CIPCoreDoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	pCmdUI->Enable(g_pIPCoreEmulator->IsReady());
...
void CMainFrame::CompileAllFile()
{
	...
	ClearMessage();
	CMultiDocTemplate* pDocTemplate = ((CIPCoreApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
	{
		CIPCoreDoc* pDoc = (CIPCoreDoc*)pDocTemplate->GetNextDoc(pos);
...
{	g_pIPCoreEmulator->Break();}
void CMainFrame::OnUpdateMenuBreak(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pIPCoreEmulator->IsReady() && g_pIPCoreEmulator->IsRunning()); }

void CMainFrame::OnMenuClearAllbreakpoint() 
{	
	g_pIPCoreEmulator->ClearBreakPoint(); 
	CMultiDocTemplate* pDocTemplate = ((CIPCoreApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
...
{	pCmdUI->Enable(g_pIPCoreEmulator->IsReady() && g_pIPCoreEmulator->IsStoped()); }

void CMainFrame::OnMenuGo() 
{	g_pIPCoreEmulator->Go(); }
...
{	pCmdUI->Enable(g_pIPCoreEmulator->IsLoad() && g_pIPCoreEmulator->IsStoped()); }
...
{	g_pIPCoreEmulator->StopEmulator(); }
...
{
	AfxGetApp()->WriteProfileString("IPCore","ProjectName",m_strProjectName);
	AfxGetApp()->WriteProfileString("IPCore","PathName",m_strPathName);
	AfxGetApp()->WriteProfileInt("IPCore","OutputListType",m_iOutListType);
