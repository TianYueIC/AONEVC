						 ...
						 m_ExpRamBar(4)
{
	...
	m_iIOtype = 32;
	g_pSSX16AEmulator = new CSSX16AEmulator(0,0,m_iIOtype);
	g_pSSX16AEmulator->GetMap(*m_pMap);
	g_pSSX16AEmulator->GetMap(*m_pOldMap);

	m_strPathName = AfxGetApp()->GetProfileString("SSX16A","PathName","C:\\Work");
	m_strPathName = AfxGetApp()->GetProfileString("SSX16A","ProjectName","DSP");
	m_iOutListType = AfxGetApp()->GetProfileInt("SSX16A","OutputListType",10);
}

CMainFrame::~CMainFrame()
{
	g_pSSX16AEmulator->KillThread();
	g_pSSX16AEmulator = NULL;
	delete m_pMap;
	delete m_pOldMap;
	delete m_pTraceOut;
	delete m_pTraceIP;
...
void CMainFrame::SendErrorMsg(CString & strMsg)
{
	*m_pOldMap = *m_pMap;
	...
	BOOL bOK;
	CString & strFileName = g_pSSX16AEmulator->GetSourceFileName(m_pMap->m_IP,bOK);
	if(bOK)
	{
		CSSX16ADoc* pDoc = (CSSX16ADoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	int file_count  = g_pSSX16AEmulator->GetSourceFileNum();
	for(int i = 0; i < file_count; i ++)
	{
		CString & strFileName = g_pSSX16AEmulator->SourceFileName(i);
		CSSX16ADoc* pDoc = (CSSX16ADoc*)AfxGetApp()->OpenDocumentFile(strFileName);
...
{
	pCmdUI->Enable(g_pSSX16AEmulator->IsReady());
...
void CMainFrame::CompileAllFile()
{
	...
	ClearMessage();
	CMultiDocTemplate* pDocTemplate = ((CSSX16AApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
	{
		CSSX16ADoc* pDoc = (CSSX16ADoc*)pDocTemplate->GetNextDoc(pos);
...
{	g_pSSX16AEmulator->Break();}
void CMainFrame::OnUpdateMenuBreak(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pSSX16AEmulator->IsReady() && g_pSSX16AEmulator->IsRunning()); }

void CMainFrame::OnMenuClearAllbreakpoint() 
{	
	g_pSSX16AEmulator->ClearBreakPoint(); 
	CMultiDocTemplate* pDocTemplate = ((CSSX16AApp*)AfxGetApp())->m_pDocTemplate;
	POSITION pos = pDocTemplate->GetFirstDocPosition();
	while (pos != NULL)
...
{	pCmdUI->Enable(g_pSSX16AEmulator->IsReady() && g_pSSX16AEmulator->IsStoped()); }

void CMainFrame::OnMenuGo() 
{	g_pSSX16AEmulator->Go(); }
...
{	pCmdUI->Enable(g_pSSX16AEmulator->IsLoad() && g_pSSX16AEmulator->IsStoped()); }
...
{	g_pSSX16AEmulator->StopEmulator(); }
...
{
	AfxGetApp()->WriteProfileString("SSX16A","ProjectName",m_strProjectName);
	AfxGetApp()->WriteProfileString("SSX16A","PathName",m_strPathName);
	AfxGetApp()->WriteProfileInt("SSX16A","OutputListType",m_iOutListType);
...
{	
	m_iIOtype = 32;
	g_pSSX16AEmulator->SetIOType(32);
}
...
{	
	m_iIOtype = 16;
	g_pSSX16AEmulator->SetIOType(16);
}
