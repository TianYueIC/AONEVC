...
void CSourceView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(egde.PtInRect(point))
	{
		...
		if( line <= m_iTotalLine) 
		{
			...
			if(!g_pSSX16AEmulator->IsReady()) return;
			
			if(g_pSSX16AEmulator->BreakPoint(m_iFileIndex,line))
				Invalidate();
		}
	}
...
void CSourceView::UpdateFirstLine()
{
	if(m_iFileIndex < 0) return;
	if(!g_pSSX16AEmulator->IsReady()) return;
	int ip = g_pSSX16AEmulator->GetIP();
	unsigned info = g_pSSX16AEmulator->GetDebugInfo(ip);
...
void CSourceView::DrawBreakPoint(CDC *pDC)
{
	...
	while(1)
	{
		unsigned info = g_pSSX16AEmulator->GetNextDebugInfo(m_iFileIndex,index);
		if(index >= 0 )
		{
...
{	if(g_pSSX16AEmulator->BreakPoint(m_iFileIndex,m_iCursorLine))Invalidate(); }
void CSourceView::OnUpdateMenuBreakPoint(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pSSX16AEmulator->IsReady() && m_iFileIndex >=0); }

void CSourceView::OnMenuGotoCursor() 
{	g_pSSX16AEmulator->GoToCursor(m_iFileIndex,m_iCursorLine);}
void CSourceView::OnUpdateMenuGotoCursor(CCmdUI* pCmdUI) 
{	pCmdUI->Enable(g_pSSX16AEmulator->IsReady() && m_iFileIndex >=0 && g_pSSX16AEmulator->IsStoped()); }
