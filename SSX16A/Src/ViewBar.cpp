...
int CViewBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    switch(m_iType)
	{
	case 1:pViewClass = RUNTIME_CLASS(CWatchView);break;
    ...
    switch(m_iType)
	{
	case 1:m_pView = DYNAMIC_DOWNCAST(CWatchView, pWnd);break;
	...
	switch(m_iType)
	{
	case 1:((CWatchView*)m_pView)->SetDSPMap(m_pMap,m_pOldMap);break;
