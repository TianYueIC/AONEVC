...
class CDSPEmluator : public CEmulator
{
    CDSPEmluator();
    CDSPEmluator(const CDSPEmluator &ref);
    CDSPEmluator &operator=(const CDSPEmluator &ref);
private:
	unsigned	m_RD[4];
	unsigned	m_RA[4];
	unsigned	m_SPmax;
...
public:
	...
	CDSPEmluator(int rom_size,int ram_size,int ioType);
	virtual ~CDSPEmluator();
...
extern CDSPEmluator * g_pEmulator;
#endif